...
public class DtoConverter {
        ...
        public UserDTO toUserDto(User user) {
                ...
                dto.setProjets(user.getProjets().stream()
                                .map(p -> new ProjetDTO(
                                                ...
                                                (p.getSiteProjet() != null) ? p.getSiteProjet().getGoogleMapsUrl()
                                                                : null,
                                                // ------------------------------------------
                                                List.of(), // documents
                ...
                dto.setInvestissements(user.getInvestissements().stream()
                                .map(inv -> {
                                        ...
                                        BigDecimal montantInvesti = prixUnePart
                                        ...
                                        return InvestissementDTO.builder()
                                                        ...
                                                        .projetId(inv.getProjet() != null ? inv.getProjet().getId()
                                                                        : null)
                                                        .projetLibelle(inv.getProjet() != null
                                                        ...
                                                        .projetPoster(inv.getProjet() != null
                                                                        ? inv.getProjet().getPoster()
                                                                        : null)
                                                        .contratUrl(inv.getContrat() != null
                                                                        ? "/api/contrats/" + inv.getContrat()
                                                                                        .getNumeroContrat()
                                                                        : null)
        ...
        public LocalisationDTO toLocalisationDto(Localisation localisation) {
                ...
                // L'ordre ici doit correspondre au record
                return new LocalisationDTO(
                                localisation.getId(), // 1. Long
                                localisation.getNom(), // 2. String
                                localisation.getAdresse(), // 3. String
                                localisation.getContact(), // 4. String
                                localisation.getResponsable(), // 5. String
                                localisation.getCreatedAt(), // 6. LocalDateTime
                                localisation.getLatitude(), // 7. BigDecimal (Géo)
                                localisation.getLongitude(), // 8. BigDecimal (Géo)
                                localisation.getWhat3words(), // 9. String (Géo)
                                localisation.getGoogleMapsUrl(), // 10. String (Géo)
                                localiteNom, // 11. String (Rel)
                                localiteId, // 12. Long (Rel)
                                paysNom, // 13. String (Rel)
                                projetNoms // 14. List (Rel)
                );
        }
        ...
        public InvestissementDTO toInvestissementDto(Investissement investissement) {
                ...
                BigDecimal prixUnePart = (investissement.getProjet() != null)
                                ? investissement.getProjet().getPrixUnePart()
                                : BigDecimal.ZERO;

                BigDecimal montantInvesti = prixUnePart
                                .multiply(BigDecimal.valueOf(investissement.getNombrePartsPris()));

                // === Contrat & Poster ===
                String projetPoster = investissement.getProjet() != null ? investissement.getProjet().getPoster()
                                : null;
                ...
                long planifies = dividendes.stream().filter(d -> d.statutDividende() == StatutDividende.PLANIFIE)
                                .count();
