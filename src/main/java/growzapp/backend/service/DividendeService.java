...
public class DividendeService {
        ...
        public void payerDividendesProjetProrata(
                ...
                if (montantTotal == null || montantTotal.compareTo(BigDecimal.ZERO) <= 0) {
                        log.warn("Montant invalide lors de la distribution pour le projet {} : {}", projetId,
                                        montantTotal);
                                ...
                                TransactionSynchronizationManager
                                                .registerSynchronization(new TransactionSynchronization() {
                                                        ...
                                                        public void afterCommit() {
                                                                // Ce code s'exécute uniquement une fois que les données
                                                                // sont réellement en
                                                                // base
                                                                log.info("Transaction commited. Lancement async facture pour dividende {}",
                                                                                dividendeId);
                                                                factureService.genererFactureEtEnvoyerEmailAsync(
                                                                                dividendeId);
                                                        }
                                                });
                        } else {
                ...
                return dividendeRepository.findByInvestissement_Projet_Id(projetId)
                                ...
                                .map(d -> {
                                        ...
                                        return new DividendeHistoriqueAdminDTO(
                                                        d.getId(),
                                                        d.getMontantTotal(),
                                                        d.getDatePaiement() != null ? d.getDatePaiement().toString()
                                                                        : null,
                                                        "Dividende prorata",
                                                        d.getInvestissement().getInvestisseur().getPrenom() + " " +
                                                                        d.getInvestissement().getInvestisseur()
                                                                                        .getNom(),
                                                        factureUrl);
                                })
